...
def process_frame(frame):
    # Ubah format gambar dari string menjadi array numpy
    np_img = np.fromstring(frame, np.uint8)
    img_cam = cv2.imdecode(np_img, cv2.IMREAD_COLOR)
    
    # img_cam = cv2.resize(img_cam, (1280, 720)) # Ukuran sudah diatur di browser
    ...
    # Logika deteksi wajah dan tangan
...
def video_feed():
    ...
    response = Response(processed_frame, mimetype='image/jpeg')
    response.headers['Content-Length'] = len(processed_frame)
